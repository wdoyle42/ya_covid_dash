...
myurl<-"https://usafactsstatic.blob.core.windows.net/public/data/covid-19/covid_deaths_usafacts.csv"
...
usa_data<-usa_data%>%
  ...
  gather(key=Date,value="Case Count",-State,-County,-Statefips,-Countyfips)%>% # Reshape
...
usa_deaths<-usa_deaths%>%
  rename(County=`County Name`)%>%
  rename_all(str_to_title)%>%
  gather(key=Date,value="Deaths",-State,-County,-Statefips,-Countyfips)%>% # Reshape
...
usa_data<-left_join(usa_data,usa_deaths,by=c("State" ,"County","Date"))%>%
mutate(County=fct_reorder(.f=as_factor(County),.x=`Case Count`,.fun = "max",.desc = TRUE))%>% # Ordered by prevalence
  group_by(State,County)%>%
  arrange(Date)%>%
  mutate(`Daily Increase, Cases`=`Case Count`-lag(`Case Count`))%>%
  mutate(`Percent Change, Cases`=(((`Case Count`-lag(`Case Count`))/`Case Count`)*100))%>%
  mutate(`Daily Increase, Deaths`=`Deaths`-lag(`Deaths`))%>%
  mutate(`Percent Change, Deaths`=(((`Deaths`-lag(`Deaths`))/`Deaths`)*100))%>%
  ungroup()%>%
  select(County,
         State,
         Date,
